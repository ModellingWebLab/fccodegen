...
class RushLarsenLabview(RushLarsenModel):
    ...
    DEFAULT_EXTENSIONS = ('.c', '.h', '.txt')

    def __init__(self, model, file_name, **kwargs):
        # fidable log1p optimisation as it's not supported by labview
        cg._optimize.LOG_OPTIMS = tuple(o for o in cg._optimize.LOG_OPTIMS if o != log1p_opt)

        self._i_inj_params = set()
        # add i_inj
        V = get_membrane_voltage_var(model, convert=False)
        i_stim = get_membrane_stimulus_current(model)
        if i_stim:
            # add units
        ...
        self._templates = ['labview.c', 'labview.h', 'labview.txt']
